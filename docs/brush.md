...
+ 上矢印ボタンと下矢印ボタンで選択しているプリセットの並び順を変更することができます
...
ブラシモード(__ぼかし__)の「__ぼかし範囲__」は、ブラシ円内の描画内容を何ピクセル単位でぼかすかを表します。
1 の場合は隣同士でぼかすのでブラシ円内がわずかにぼける程度になります。
5 の場合は 5 px の範囲でぼかすので、より広範囲でぼかすことになります。

ブラシモード(__指先__)の「__ぼかし範囲__」も同じく、ブラシ円内の描画内容を何ピクセル単位でぼかすかを表します。
1 の場合は隣同士でぼかすのでブラシ円内がわずかにぼける程度になります。
5 の場合は 5 px の範囲でぼかすので、より広範囲でぼかすことになります。
...
「ツール」ウィンドウの中の「__強さ__」は 100 の場合は、ブラシベース画像の濃淡をそのまま適用します。
